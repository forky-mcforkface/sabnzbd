...
def process_job(nzo: NzbObject):
    ...
    try:
        ...
        if all_ok:
            ...
            # Use dirs already generated by direct-unpacker
            if not nzo.unpack_dir_info:
                # Generate extraction path
                nzo.prepare_extraction_path()

            tmp_workdir_complete = nzo.unpack_dir_info.tmp_workdir_complete
            workdir_complete = nzo.unpack_dir_info.workdir_complete
            ...
            if flag_unpack:
                ...
                unpack_error, newfiles = unpacker(nzo, tmp_workdir_complete, nzo.unpack_dir_info.one_folder)
            ...
            if all_ok and nzo.unpack_dir_info.marker_file:
                sabnzbd.filesystem.del_marker(os.path.join(tmp_workdir_complete, nzo.unpack_dir_info.marker_file))
                remove_from_list(nzo.unpack_dir_info.marker_file, newfiles)
        ...
        if not nzb_list:
            ...
            if cfg.folder_rename() and tmp_workdir_complete and not nzo.unpack_dir_info.one_folder:
                if not all_ok:
                ...
                try:
                    ...
                    nzo.unpack_dir_info.file_sorter.sorter_active = None
            ...
            if all_ok and nzo.unpack_dir_info.file_sorter.sorter_active:
                if newfiles:
                    workdir_complete, ok = nzo.unpack_dir_info.file_sorter.sorter.rename(newfiles, workdir_complete)
                    if not ok:
